...
module.exports = class SmartMeter extends EventEmitter {
  ...
  connect () {
    ...
    self._connection.on('data', line => {
      ...
      if (line[0] === '!') {
        // DSMR5 publishes new telegram every second and slow computers seem to have
        // issues parsing that fast resulting in data corruption. This prevents processing
        // of a new telegram when processing of the previous one has not been finished
        if (!self._inProgress) {
